//! `zag-audit` — the tool that checks this repository against its own claims.
//!
//! These commands were in `zag` itself, which made the workbench binary carry
//! the standards registry, the plain-language rules, the concept system and the
//! conformance evidence generator. None of that is needed to record a shell
//! session or ask a model a question, and a person opening a terminal should
//! not have to scroll past thirteen commands that audit documentation to find
//! the one that opens a shell.
//!
//! The split is by what a command looks at. `zag` looks at a workspace: its
//! record, its policy, its shell, its history. `zag-audit` looks at a
//! repository: its text, its standards, its generated files, its evidence.
//! Nothing was rewritten in the move.
//!
//! `zag check` is the release gate and is still the release gate; it is just
//! spelled `zag-audit check` now, and `zig build check` runs it either way.
...
const Command = enum {
    ...
    fn parse(text: []const u8) ?Command {
        ...
        for (table) |entry| {
            if (std.mem.eql(u8, text, entry.name)) return entry.command;
        }
        return null;
    }
};
...
pub const help_text =
    \\zag-audit - check a repository against the standards it claims to meet.
    \\
    \\Use it like this:
    \\  zag-audit <command> [options]
    \\
    \\Commands
    ...
    \\
    \\To record work rather than audit it, use "zag". Run "zag help" to see it.
    \\
;
...
pub fn main(init: std.process.Init) !u8 {
    ...
    const command = Command.parse(argv[1]) orelse {
        try w.print("\"{s}\" is not a zag-audit command.\n\n", .{argv[1]});
        try w.writeAll(help_text);
        try w.flush();
        return 2;
    };
    ...
    const options = parseOptions(arena, text_args.items) catch |err| {
        switch (err) {
            error.MissingOptionValue => try w.writeAll("An option is missing its value. Run \"zag-audit help\" to see the options.\n"),
            error.UnknownOption => try w.writeAll("That option is not one zag-audit knows. Run \"zag-audit help\" to see the options.\n"),
            else => return err,
        }
        try w.flush();
        return 2;
...
fn workbenchActor() zag.events.event.Actor {
    return zag.events.event.Actor.of(zag.core.identity.program("zag-audit"));
}
...
fn evidenceReport(arena: std.mem.Allocator, io: std.Io, w: *std.Io.Writer, options: Options) !u8 {
    ...
    const tool: zag.standards.evidence.ToolIdentity = .{ .name = "zag-audit check", .version = zag.version };
...
fn emit(arena: std.mem.Allocator, io: std.Io, w: *std.Io.Writer, options: Options) !u8 {
    ...
    const openapi_body = try zag.interop.openapi.documentAlloc(&zag.daemon.interface.daemon_operations, arena, .{
    ...
    try glossary.writer.writeAll(
        \\# Glossary
        \\
        \\Generated by `zag-audit emit` from the concept system in `src/knowledge`. Do not
    ...
    try risks.writer.writeAll(
        \\# Risk register
        \\
        \\Generated by `zag-audit emit` from `src/ai/risk.zig`. A treatment that claims to
