...
| --- | --- | --- | --- | --- |
| Linux | x86-64 | Debug and ReleaseSafe builds in the release gate | All automated tests run; the pseudoterminal command path runs in tests | Development platform |
| macOS | ARM64 | ReleaseSafe cross-compile in continuous integration | No native test run; no macOS pseudoterminal implementation | Compile-only evidence |
| Windows | x86-64 | ReleaseSafe cross-compile in continuous integration | No native test run; no ConPTY implementation | Compile-only evidence |
